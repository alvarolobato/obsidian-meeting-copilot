...
The cast is real: **<demo-domain>**, the group **`demo-group@<demo-domain>`**, and its
...
| --- | --- | --- | --- |
| Signed-in user — **not** in the group | `demo-user@<demo-domain>` | — | — |
| The group | `demo-group@<demo-domain>` | — | "Demo Group" |
...
### Two traps when choosing the cast

**1. Don't use a local part that already reads as the real name.** With no scope
granted the app falls back to `humanizeEmailName()`, so `sophie.chen@` renders as
"Sophie Chen" — character-for-character identical to the directory name, and granting
`directory.readonly` would change nothing on screen.

A local part that is merely a *shortened* form of the real name is nearly as bad:
`dave@` for "Dave Whitfield" guesses "Dave", so the row only gains a surname, which
a reviewer can read as cosmetic. Initial-style local parts (`schen@` → "Schen") keep
the guess visibly wrong. **The gap between the guess and the real name is the entire
evidence of the video** — pick addresses that maximise it.

**2. Don't sign in as a member of the group.** If the demo account is itself in the
group, Calendar may carry it on the event as its own attendee entry with a display name
attached. The baseline then shows **two rows, one already correctly named**, instead of
the single nameless row the whole shot list is built on — which is close to the reason
the first submission was rejected.

Sign in as an account outside the group. Either way, the provisioning script's
`checkBaseline()` prints the guest list exactly as the Calendar API returns it and tells
you PASS or FAIL. **Run it before recording.** It passes when there is exactly one
attendee, `demo-group@<demo-domain>`, with no `displayName`.
...
With both toggles off and cache bypass on, the agenda must show a single **"Demo Group"**
...
- Open the agenda and show **"Q3 planning review"**. Its guest list is a single row:
  **"Demo Group"**. The meeting has three actual participants and the app can name
  none of them.
...
- The same three rows become **"Sophie Chen"**, **"Raj Patel"**, **"Mia Okafor"**.
  **Linger on whichever row changes most** — a guess that is merely a shortened
  form of the real name reads as cosmetic; one that is plainly wrong does not
  (see §6).
...
- [ ] Baseline really showed one "Demo Group" row — not three people, and not a
      second row for Schen (`checkBaseline()` PASSes).
- [ ] After the group grant, rows really read "Schen"/"Rpatel"/"Mokafor" — not
