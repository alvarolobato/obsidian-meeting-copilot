...
1. In *Settings → Meeting Copilot → General → Google Calendar*, click **Authenticate** and grant access in the browser. The plugin ships with a built-in Google app, so there is nothing to create in the Google Cloud Console.
2. Optionally set the **Target calendar ID** (defaults to `primary`) and the agenda's **horizon** / **history** window.

Some setups need your own OAuth client instead: your organization blocks third-party apps, or you'd rather run the optional permissions below off a client of your own. Both are covered in [Bring your own Google credentials](google-credentials.md) — the resulting **Client ID** and **Client secret** go under *Advanced: custom OAuth credentials* in the same section, before you authenticate.
...
Auth also requests two **optional** scopes, each toggleable independently under *Settings → Meeting Copilot → General → Google Calendar → Optional permissions* (both on by default). Their Google verification is still in review, so granting them can show a "Google hasn't verified this app" screen. They only improve attendee display names on the agenda — calendar access itself never needs them:
