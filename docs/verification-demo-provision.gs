 ...
 *
 * The demo signs in as DEMO_USER, who must NOT be a member of the group — see
 * `checkBaseline` at the bottom for why.
 */

var DOMAIN = "<demo-domain>";
var GROUP = "demo-group@" + DOMAIN;

/** The signed-in demo user. Must NOT be in MEMBERS — see checkBaseline. */
var DEMO_USER = "demo-user@" + DOMAIN;
 ...
 */
var MEMBERS = [
  { email: "schen@" + DOMAIN,   given: "Sophie", family: "Chen",   guess: "Schen"   },
  { email: "rpatel@" + DOMAIN,  given: "Raj",    family: "Patel",  guess: "Rpatel"  },
  { email: "mokafor@" + DOMAIN, given: "Mia",    family: "Okafor", guess: "Mokafor" },
];
 ...
 *
 * If the signed-in account is itself in the group, Calendar may carry it on the
 * event as its own attendee entry (with a displayName). The baseline then shows
 * TWO rows — one already correctly named — instead of the single nameless
 * "Demo Group" row the shot list depends on.
 *
 * PASS: exactly one attendee, demo-group@<demo-domain>, with no displayName.
...
function checkBaseline() {
  ...
  if (ok) {
    Logger.log("PASS  one nameless group row — the baseline shot will read \"Demo Group\".");
  } else {
